...
export const baseSkuRouter = createTRPCRouter({
			...
			try {
				...
				try {
					[colorAttr] = await db
						...
						.limit(1);
					console.log(
						"[listBaseProducts] Color attribute found:",
						colorAttr?.id,
					);
				} catch (err) {
					console.error(
						"[listBaseProducts] Color attribute query failed:",
						err,
					);
					colorAttr = null;
				}
              ...
              WHERE bs.category_id = ${categories.id}
				...
				const products = pageRows.map((p) => {
					...
					return {
						...
						frontMockupUrl: frontById[p.id]
							? getPublicUrl(frontById[p.id])
							: "",
					};
				});
	...
	getProductCategories: publicProcedure.query(async () => {
		...
		try {
			console.log(
				"[getProductCategories] Fetching categories with product counts",
			);
			let rows;
			try {
				rows = await db
              ...
              WHERE bs.category_id = ${categories.id}
				...
				throw new Error(
					`Categories query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
				);
			}
			...
			console.log(
				`[getProductCategories] Success! Returning ${out.length} categories in ${Date.now() - startTime}ms`,
			);
			return out;
		} catch (err) {
			...
			try {
				...
				try {
					[product] = await db
						...
						.innerJoin(categories, eq(baseSkus.categoryId, categories.id))
						.where(
							and(eq(baseSkus.id, input.id), eq(baseSkus.status, "active")),
						)
						.limit(1);

					console.log(
						"[getBaseProductById] Product found:",
						product ? product.name : "NOT FOUND",
					);
				} catch (err) {
					console.error("[getBaseProductById] Product query failed:", err);
					throw new Error(
						`Product query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					);
				}
				...
				try {
					[colorAttr] = await db
						...
						.limit(1);
					console.log(
						"[getBaseProductById] Color attribute found:",
						colorAttr?.id,
					);
				} catch (err) {
					console.error(
						"[getBaseProductById] Color attribute query failed:",
						err,
					);
					colorAttr = null;
				}
				...
				if (colorAttr) {
					...
					try {
						[rule] = await db
							...
							.limit(1);
						console.log(
							"[getBaseProductById] Color rule found:",
							rule?.valueSetId,
						);
					} catch (err) {
					...
					if (rule?.valueSetId) {
						...
						try {
							...
							console.error(
								"[getBaseProductById] Color values query failed:",
								err,
							);
							throw new Error(
								`Color values query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
							);
						}
				...
				try {
					rows = await db
						...
						.orderBy(attributes.code);
					console.log(
						`[getBaseProductById] Found ${rows.length} attribute entries`,
					);
				} catch (err) {
					console.error("[getBaseProductById] Attributes query failed:", err);
					throw new Error(
						`Attributes query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					);
				}
				...
				console.log(
					`[getBaseProductById] Success! Returning product ${product.name} in ${Date.now() - startTime}ms`,
				);
				return result;
			} catch (err) {
				const elapsed = Date.now() - startTime;
	...
	getListingProducts: publicProcedure
		...
		.query(async ({ input }) => {
			try {
				...
				const frontDataById = frontViewData.reduce<
					Record<string, (typeof frontViewData)[0]>
				>((acc, row) => {
				...
				const products = pageRows.map((p) => {
					...
					return {
						...
						colors: colorInfo?.colors ?? [],
						totalColors: colorInfo?.totalColors ?? 0,
						frontMockupUrl: frontData?.mockupR2Key
							? getPublicUrl(frontData.mockupR2Key)
							: "",
						frontPrintArea: frontData?.printAreaId
							? {
									...
									sourceWidthPx:
										frontData.printAreaSourceWidthPx ??
										frontData.sourceWidthPx ??
										400,
									sourceHeightPx:
										frontData.printAreaSourceHeightPx ??
										frontData.sourceHeightPx ??
										400,
			...
			try {
				...
				try {
					[baseSku] = await db
						...
						.where(
							and(
								eq(baseSkus.id, input.baseSkuId),
					...
					console.log(
						"[getEditorData] Base SKU found:",
						baseSku ? baseSku.name : "NOT FOUND",
					);
				} catch (err) {
					console.error("[getEditorData] Base SKU query failed:", err);
					throw new Error(
						`Base SKU query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					);
				}
				...
				try {
					...
					throw new Error(
						`Views query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					);
				}
				...
				try {
					...
					console.log(
						`[getEditorData] Found ${mockupsData.length} editor mockups`,
					);
				} catch (err) {
					console.error("[getEditorData] Mockups query failed:", err);
					throw new Error(
						`Mockups query failed: ${err instanceof Error ? err.message : "Unknown error"}`,
					);
				}
				...
				for (const row of viewsData) {
					if (!viewsMap.has(row.viewId)) {
						viewsMap.set(row.viewId, {
							...
							mockupImageUrl: mockupsMap.get(row.viewId)
								? getPublicUrl(mockupsMap.get(row.viewId)!)
								: null,
							printArea: null,
						});
					}
				...
				console.log(
					`[getEditorData] Success! Returning editor data for ${baseSku.name} in ${Date.now() - startTime}ms`,
				);
				return result;
			} catch (err) {
				const elapsed = Date.now() - startTime;
	...
	getProductForDesigner: publicProcedure
		...
		.query(async ({ input }) => {
			const startTime = Date.now();
			console.log(
				"[getProductForDesigner] Starting for baseSkuId:",
				input.baseSkuId,
			);
			...
			try {
				...
				console.log(
					`[getProductForDesigner] Completed in ${Date.now() - startTime}ms`,
				);
