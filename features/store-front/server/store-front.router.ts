...
export const storeFrontRouter = createTRPCRouter({
	getProductDetails: publicProcedure
		...
		.query(async ({ input }) => {
			try {
					...
					with: {
						...
						variants: {
							columns: {
								...
								sku: true,
								price: true,
								primaryMockupR2Key: true,
								secondaryMockupR2Key: true,
								comboHash: true,
							},
							with: {
								attributeValues: {
						...
						baseSku: {
							...
							with: {
								...
								views: {
									where: eq(baseSkuViews.code, "front"),
									columns: {},
									limit: 1,
									with: {
										mockups: {
											where: eq(baseSkuMockups.purpose, "editor_background"),
											columns: {
												r2Key: true,
											},
											limit: 1,
				...
				console.log("PRoduct", result.variants[0].attributeValues);

				const product = {
					id: result.id,
					title: result.baseSku.name,
					price: Number(result.price),
					listingName: result.listing.title,
					templateUrl: getPublicUrl(result.baseSku.views[0].mockups[0].r2Key),
					specs: result.baseSku.attributeRules
						.filter(({ value }) => !!value)
						.map(({ value, attribute }) => ({
							...attribute,
							value: value?.displayName,
						})),
					//just for show will fix this
					sizes:
						result.baseSku.attributeRules.filter(
					...
					variants: [
						...result.variants.map((variant) => ({
							...
							color: variant.attributeValues
								.filter(({ attribute }) => attribute.dataType === "color")
								.map(({ attributeValue }) => attributeValue)[0],
							size: variant.attributeValues
								.filter(({ attribute }) => attribute.code === "size")
								.map(({ attributeValue }) => ({
									code: attributeValue.code,
									displayName: attributeValue.displayName,
								}))[0],
							images: [
								{
									url:
										variant.primaryMockupR2Key &&
										getPublicUrl(variant.primaryMockupR2Key),
									isDefault: true,
								},
								{
									url:
										variant.secondaryMockupR2Key &&
										getPublicUrl(variant.secondaryMockupR2Key),
									isDefault: true,
								},
							],
						})),
	...
	getStoreBySlug: publicProcedure
		...
		.query(async ({ input }) => {
			const store = await db
				...
				.where(
					and(eq(stores.storeSlug, input.slug), eq(stores.status, "active")),
				)
				.limit(1);
	...
	getStoreProducts: publicProcedure
		...
		.query(async ({ input }) => {
			...
			try {
				console.log(
					`[getStoreProducts] Starting query for store: ${input.storeSlug}`,
					{
						search: input.search,
				...
				if (input.categorySlug) {
					whereConditions.push(eq(categories.slug, input.categorySlug));
				}
				...
				if (rawProducts.length === 0) {
					console.log(
						`[getStoreProducts] No products found for store: ${input.storeSlug}`,
					);
					return {
						products: [],
						pagination: {
				...
				const variants = await db
					...
					.innerJoin(
						variantAttributeValues,
						eq(productVariants.id, variantAttributeValues.variantId),
					)
					.innerJoin(
						attributeValues,
				...
				defaultMockups.forEach((mockup) => {
					defaultMockupsByBaseSku.set(
						mockup.baseSkuId,
						getPublicUrl(mockup.r2Key),
					);
				});
				...
				const processedProducts = rawProducts.map((product) => {
					const productVariants =
						variantsByProduct.get(product.productId) || [];
					...
					const variantWithMockup = productVariants.find(
						(v) => v.primaryMockupR2Key,
					);
					if (variantWithMockup?.primaryMockupR2Key) {
						defaultImageUrl = getPublicUrl(
							variantWithMockup.primaryMockupR2Key,
						);
					} else {
						const defaultMockup = defaultMockupsByBaseSku.get(
							product.baseSkuId,
						);
						if (defaultMockup) {
				...
				console.log(
					`[getStoreProducts] Success in ${elapsed}ms for store: ${input.storeSlug}`,
					{
