...
export function PickBaseView({ className }: Props) {
  ...
  const { data, isLoading, isError } =
    trpc.productDesign.getBaseCatalog.useQuery(
      {
        query,
  ...
  return (
    <div className={cn("container mx-auto py-8 px-4 max-w-7xl", className)}>
      {/** Header */}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 gap-y-16">
              {data.products.map((product) => (
                <BaseCard
                  key={product.id}
                  base={{ ...product, displayImageUrl: product.heroImageUrl }}
                />
              ))}
            </div>
          </section>
