...
export function BaseCard({
	...
	const isInListing = useProductDesignStore((state) =>
		state.listing.products.includes(base.id),
	);
			...
			<Card className={cn("p-0 border-none overflow-hidden")}>
				<div className="aspect-square relative overflow-hidden isolate">
					...
					{!isPreEditor && (
						<div
							className={cn(
								"absolute z-20 top-4 right-4  ring-1 rounded-sm size-6 grid place-items-center",
								isInListing
									? " ring-foreground bg-primary"
					...
					{!isPreEditor ? (
						<p className="test-sm">
							...
							<span className="font-medium text-green-500">
								{formatCurrency(
									Number(base.cost) * 1.2 - Number(base.cost),
									"UGX",
								)}
							</span>
						</p>
