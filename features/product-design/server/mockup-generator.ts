...
export const extractColorProfileAndSize = async (
	designR2Key: string,
): Promise<{
	...
	try {
		const designBuffer = await downloadFile("PUBLIC", designR2Key);
...
export const generateSingleMockup = async (
	...
	const [designBuffer, templateBuffer] = await Promise.all([
		await downloadFile("PUBLIC", designR2Key),
		await downloadFile("PUBLIC", templateR2Key),
	]);
	...
	if (visibleDesign.length > 0) {
		...
		if (needsClipping) {
			...
			return {
				mockupBuffer: isClient
					? finalComposite.toString("base64")
					: finalComposite,
				metadata: {
	...
	return {
		mockupBuffer: isClient ? finalComposite.toString("base64") : finalComposite,
		metadata: {
			width: metadata.width,
			height: metadata.height,
			size: finalComposite.length,
...
export const generateBulkMockups = async (
	...
	const { designR2Key, placement, variants, onProgress } = options;
			...
			chunk.map(async (variant) => {
				try {
					const result = await generateSingleMockup({
						designR2Key,
						templateR2Key: variant.templateR2Key,
