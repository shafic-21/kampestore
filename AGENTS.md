...
- `features/`: Domain modules (e.g., creators), each with `ui/`, `server/`, and `store/` subfolders.
- `components/`: Reusable UI components.
- `server/db/schema/`: Drizzle ORM table definitions (Postgres).
- `server/db/migrations/`: SQL migrations managed by Drizzle.
- `lib/`: Utilities (e.g., `r2.ts`).
- `public/`: Static assets.

## Build, Test, and Development Commands
- `pnpm dev`: Start Next.js in dev mode (uses Turbopack).
- `pnpm build`: Compile the production build.
- `pnpm start`: Run the compiled app.
- `pnpm lint`: Lint code with ESLint.
- `pnpm typecheck`: Type-check with TypeScript.
- `pnpm db:migrate`: Apply DB migrations.
- `pnpm db:push`: Push schema changes (use with care).
...
- Linting: ESLint (`eslint-config-next`). Keep code warning-free.
- Styles: Tailwind CSS v4. Prefer utility classes over custom CSS.
- Indentation: 2 spaces; avoid long lines (>120 chars).
- Naming: PascalCase for React components; camelCase for variables/functions; route segments kebab-case; DB tables/columns snake_case (map to camelCase in code via Drizzle models).
- Files: Co-locate by feature under `features/<domain>/`.

## Testing Guidelines
- No test runner is configured yet. Use `pnpm typecheck` and `pnpm lint` as a baseline gate.
- When adding tests, prefer Vitest for unit and Playwright for e2e; place tests alongside code (`*.test.ts`/`*.spec.ts`). Keep fast and deterministic.

## Commit & Pull Request Guidelines
- Commits: Imperative, concise messages (e.g., `feat(editor): add color to mockups`). Group related changes.
- PRs: Include summary, screenshots (UI changes), linked issues, migration notes (if touching DB), and manual test steps.
- DB changes: Use forward-only migrations; avoid destructive changes. Ensure app code is compatible with both pre- and post-migration during rollout.

## Security & Configuration Tips
- Secrets: Use `.env.local`; never commit secrets. See usages in `server/` and `lib/`.
- Database: Neon serverless Postgres via Drizzle. Run migrations before starting the app.
- Storage: R2 keys live in env vars; verify bucket names before uploading assets.
