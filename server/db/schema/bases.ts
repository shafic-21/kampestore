import { pgTable, text, uuid, timestamp, bigint, json, integer, boolean, index, unique, check } from "drizzle-orm/pg-core";
import { relations } from "drizzle-orm";
import { categories, attributes, attributeValueSets, attributeValues } from "./catalog";
import { sql } from "drizzle-orm";


// Base product templates
export const baseSkus = pgTable("base_skus", {
...
export const baseSkuAttributeRules = pgTable("base_sku_attribute_rules", {
  id: uuid("id").defaultRandom().primaryKey(),
  ...
  unique("base_sku_attribute_rules_unique").on(table.baseSkuId, table.attributeId),
]);

// Product views (front, back, etc.)
export const baseSkuViews = pgTable("base_sku_views", {
  id: uuid("id").defaultRandom().primaryKey(),
  baseSkuId: uuid("base_sku_id").notNull().references(() => baseSkus.id, { onDelete: "cascade" }),
  ...
  unique("base_sku_views_base_sku_code_unique").on(table.baseSkuId, table.code),
]);

// Print areas per view
export const baseSkuPrintAreas = pgTable("base_sku_print_areas", {
  id: uuid("id").defaultRandom().primaryKey(),
  baseSkuId: uuid("base_sku_id").notNull().references(() => baseSkus.id, { onDelete: "cascade" }),
  viewId: uuid("view_id").notNull().references(() => baseSkuViews.id, { onDelete: "cascade" }),
...
export const baseSkuMockups = pgTable("base_sku_mockups", {
  id: uuid("id").defaultRandom().primaryKey(),
  baseSkuId: uuid("base_sku_id").notNull().references(() => baseSkus.id, { onDelete: "cascade" }),
  viewId: uuid("view_id").notNull().references(() => baseSkuViews.id, { onDelete: "cascade" }),
  colorValueId: uuid("color_value_id").notNull().references(() => attributeValues.id, { onDelete: "cascade" }),
  ...
  unique("base_sku_mockups_base_view_color_unique").on(table.baseSkuId, table.viewId, table.colorValueId),
]);
...
export const baseSkuAttributeRulesRelations = relations(baseSkuAttributeRules, ({ one }) => ({
...
export const baseSkuViewsRelations = relations(baseSkuViews, ({ one, many }) => ({
  baseSku: one(baseSkus, {
...
export const baseSkuPrintAreasRelations = relations(baseSkuPrintAreas, ({ one }) => ({
  baseSku: one(baseSkus, {
